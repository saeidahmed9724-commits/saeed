...
// --- DIRECT-TO-SUPABASE UPLOAD (bypasses Vercel's ~4.5MB serverless body limit) ---
//
// Why this exists: the /api/upload endpoint above sends the whole file as base64
// inside the JSON body. Vercel enforces a hard ~4.5MB request-body limit on
// Serverless/Node Functions at the platform level (express's `limit: '100mb'`
// has no effect on this — it's rejected before your code even runs). Base64
// adds ~33% overhead, so only raw files under ~3.3MB can ever get through.
// That's why single small photos usually work, several photos batched together
// often fail, and songs/videos (which are almost always bigger than that)
// basically never work.
//
// The fix: the browser uploads the raw file straight to Supabase Storage using
// a short-lived signed upload URL, so the file bytes never pass through the
// Vercel function at all. Only small JSON (paths/metadata) touches /api/upload/*.

// Step 1: mint one signed upload URL per file (does NOT touch file bytes)
app.post('/api/upload/sign', async (req, res) => {
  const { role, category, files } = req.body as {
    role?: string;
    category?: string;
...
// Step 2 happens client-side: the browser uploads each file directly to Supabase
// using `supabase.storage.from(bucket).uploadToSignedUrl(path, token, file)`.
// See uploadService.ts for the client implementation.

// Step 3: once direct uploads succeed, register the small metadata (no file bytes)
app.post('/api/upload/complete', async (req, res) => {
  const { role, category, title, description, date, artist, items } = req.body as {
    role?: 'Dodo' | 'SO';
    category?: string;
    title?: string;
    description?: string;
  ...
  if (!category || !Array.isArray(items) || items.length === 0) {
    return res.status(400).json({ error: 'Missing category or uploaded items' });
  }
  ...
  try {
    ...
    console.error('Error completing direct upload:', err);
